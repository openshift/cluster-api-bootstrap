...
MUTABLE_TAG ?= latest
IMAGE       ?= cluster-api-provider-openshift
...
.PHONY: build
build: generate fmt vet bin/bootstrap bin/controlplane## Build manager binary.
	
bin/bootstrap:
	go build -o bin/bootstrap ./bootstrap/cmd/

bin/controlplane:
	go build -o bin/controlplane ./bootstrap/cmd/
